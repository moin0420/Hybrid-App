  ...
  broadcastTimers.set(
    ...
    setTimeout(async () => {
      try {
        ...
        console.error("Failed broadcasting requisitions:", err);
      }
      broadcastTimers.delete(key);
    }, 1200)
  );
...
app.put("/api/requisitions/:requirementId", async (req, res) => {
  ...
  try {
    ...
    // Normal edits - preserve other fields when undefined
    const updates = {
      client: client !== undefined ? client : row.client,
      title: title !== undefined ? title : row.title,
      status: status !== undefined ? status : row.status,
...
io.on("connection", (socket) => {
  ...
  // Forward editing_status messages: broadcast to others that someone is editing a specific field
  socket.on("editing_status", (payload) => {
    // payload: { requirementId, field, userName, isEditing }
    // rebroadcast to other clients
    socket.broadcast.emit("editing_status", payload);
  });
